...
import '../cubit/parent_notifications_cubit.dart';

/// Legacy `/parent` portal "Alerts" tab. Wired to `GET /notifications`; tapping
/// an unread notification marks it read (`POST /notifications/{id}/read`).
class ParentNotificationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => ParentNotificationsCubit(sl<NotificationRepository>()),
      child: const _ParentNotificationsView(),
    );
  }
}

class _ParentNotificationsView extends StatelessWidget {
  const _ParentNotificationsView();
  ...
  Widget build(BuildContext context) {
    ...
    return SchooKeepScaffold(
      ...
      body: BlocBuilder<ParentNotificationsCubit, DataState<List<AppNotification>>>(
        ...
        child: Column(
          ...
          children: [
            ...
            SchooKeepButton(
              ...
              fullWidth: false,
              onPressed: () => context.read<ParentNotificationsCubit>().load(),
            ),
          ],
        ),
      ),
  ...
  Widget _content(BuildContext context, List<AppNotification> items, bool isRTL) {
    ...
    return RefreshIndicator(
      onRefresh: () => context.read<ParentNotificationsCubit>().load(),
      child: ListView(
  ...
  Widget _notificationCard(BuildContext context, AppNotification n, bool isRTL) {
    ...
    return SchooKeepCard(
      ...
      onTap: n.isUnread
          ? () => context.read<ParentNotificationsCubit>().markRead(n.id)
          : null,
      child: card,
    );
  }
