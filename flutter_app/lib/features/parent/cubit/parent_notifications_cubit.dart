...
/// Loads the authenticated user's in-app notifications (`GET /notifications`)
/// and marks them read (`POST /notifications/{id}/read`).
class ParentNotificationsCubit extends Cubit<DataState<List<AppNotification>>> {
  ParentNotificationsCubit(this._repo) : super(const DataLoading()) {
    load();
  }
  ...
  /// Marks a single notification read and updates it in place (optimistic on
  /// the server response).
