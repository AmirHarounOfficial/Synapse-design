...
/// Ported from `BusStudentBoarding.tsx`, now wired to
/// `POST /bus-routes/{routeId}/events`. Confirming opens a two-state dialog
/// (confirm → sending) that records the boarding event, then returns to the
/// route overview. Student details come from the tapped manifest row.
class BusStudentBoardingScreen extends StatelessWidget {
  const BusStudentBoardingScreen({
    super.key,
    required this.id,
    this.routeId,
    this.studentName,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'When you confirm boarding, the parent will receive an automatic push notification: "Your child $firstName has boarded the bus at $_currentTime"',
...
class _BoardingDialog extends StatelessWidget {
  const _BoardingDialog({required this.name, required this.currentTime, required this.onConfirm});
  final String name;
  ...
  Widget _confirmState(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text('Confirm Student Boarding',
        ...
        RichText(
          ...
          text: TextSpan(
            ...
            children: [
              ...
              TextSpan(text: ' as boarded at $currentTime?'),
            ],
          ),
        ),
        const SizedBox(height: 24),
